  ...
  return (
    <div className="p-6 border rounded shadow">
      <div className="flex items-center justify-between">
        <img className="h-8" src={job.companyId
.image} alt="" />
      </div>
      <h4 className="mt-2 text-xl font-medium">{job.title}</h4>
      <div className="flex items-center gap-3 mt-2 text-xs">
        <span className="px-4 border bg-blue-50 border-blue-200 py-1.5 rounded">{job.location}</span>
        <span className="px-4 border bg-red-50 border-red-200 py-1.5 rounded">{job.level}</span>
      </div>
