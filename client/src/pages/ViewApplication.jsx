...
const ViewApplication = () => {
  ...
  // function to updated the job application status
            ...
            {applicants.filter(item => item.jobId && item.userId).map((applicant,index)=>(
              <tr key={index} className='text-gray-700'>
                ...
                <td className='px-4 py-2 border-b'>
                  <a href={applicant.userId.resume} target='_blank' className='inline-flex items-center gap-2 px-3 py-1 text-blue-400 rounded bg-blue-50'>Resume
                ...
                <td className='relative px-4 py-2 border-b'>
                  ...
                  ? <div className='relative inline-block text-left group'>
                    ...
                    <div className='absolute top-0 right-0 z-10 hidden w-32 mt-2 bg-white border border-gray-200 rounded shadow md:left-0 group-hover:block'>
                      <button onClick = {()=>{changeJobApplicationStatus(applicants._id,'Accepted')}} className='block w-full px-4 py-2 text-left text-blue-500 hover:bg-gray-100 '>Accept</button>
                      <button onClick = {()=>{changeJobApplicationStatus(applicants._id,'Rejected')}} className='block w-full px-4 py-2 text-left text-red-500 hover:bg-gray-100 '>Reject</button>
                    </div>
                  </div>
